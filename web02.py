...
def index():
    if 'uid' in session:
        # Jalankan virus.py dan ambil outputnya
        result = subprocess.run(
            ['python', 'injectVirus/virus.py'],
            capture_output=True, text=True
        )
        virus_message = result.stdout.strip().replace('"', '\\"')
        tl = get_time_lines()
        return Template('''
<!doctype html>
<html lang="en">
...
<body>
  ...
  <script>
    // Pesan dari virus.py akan muncul di Console browser
    console.log("{{ virus_message }}");
  </script>
</body>
</html>
